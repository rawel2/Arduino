...
void ColorduinoObject::ColorFill_noFlip(unsigned char R,unsigned char G,unsigned char B)
{
  PixelRGB *p = GetPixel(0,0);
  for (unsigned char y=0;y<ColorduinoScreenWidth;y++) {
    for(unsigned char x=0;x<ColorduinoScreenHeight;x++) {
...
void ColorduinoObject::RectangleFill_noFlip(unsigned char x0, unsigned char y0,unsigned char x1, unsigned char y1,unsigned char R,unsigned char G,unsigned char B)
{
	if((x0<ColorduinoScreenHeight) and (y0<ColorduinoScreenWidth) and (x1<ColorduinoScreenHeight) and (y1<ColorduinoScreenWidth) and (x0<=x1) and (y0<=y1))
	{
		
		for (unsigned char y=y0;y<=y1;y++) {
			PixelRGB *p = GetPixel(x0,y);
