...
void setup()
{
  ...
  // whiteBalVal[1]=green
  // whiteBalVal[2]=blue
  unsigned char whiteBalVal[3] = {30,63,60}; // for LEDSEE 6x6cm round matrix
    ...
    if (c == 0){
      for (byte x = 0; x < 8; x++){
        for (byte y = 0; y < 8; y++){
           ...
           // PixelRGB *p = Colorduino.curWriteFrame + (y * 8) + x;
           // &((*Colorduino.GetDrawPixel)[x][y]);
           p->r = Wire.read();
           p++;
        }
      }
